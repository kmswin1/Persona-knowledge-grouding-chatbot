...
BART_CTXT_MODEL_INPUTS = ["input_ids", "input_eos", "decoder_input_ids", "lm_labels", "token_type_ids", "mc_token_ids",
                          "persona_candidates", "persona_can_idx", "persona_grounding",
                          "knowledge_candidates", "knowledge_can_idx", "knowledge_grounding",
                          "tot_knowledge", "tot_knowledge_eos", "reply", "dialog", "filtered_triples",
                          # "filtered_triples_raw",
                          "ner_cans"]
...
def pad_dataset_bart_inctxt(dataset, padding=1):
    ...
    max_l_tot_knowledge = max([len(i) for x in new_dataset["tot_knowledge"] for i in x])
    max_l_persona_cans = max([len(i) for x in new_dataset["persona_candidates"] for i in x])
    max_l_triple_cans = max([len(i) for x in new_dataset["filtered_triples"] for i in x])
    # max_l_triple_plus_knowcans = max([len(i) for x in new_dataset["triple_plus_knowledge"] for i in x])
    # max_l_triple_raw = max([len(i) for x in new_dataset["filtered_triples_raw"] for i in x])
    max_l_ner_cans = max([len(i) for x in new_dataset["ner_cans"]])
    max_l_dialog = max(len(x) for x in new_dataset["dialog"])
    ...
    # Using padding token to fit knowledge candidate repr
    new_dataset["filtered_triples"] = [[row + [padding] * (max_l_triple_cans - len(row)) for row in x] for x in new_dataset["filtered_triples"]]
    # new_dataset["filtered_triples_raw"] = [[row + [padding] * (max_l_triple_raw - len(row)) for row in x] for x in new_dataset["filtered_triples_raw"]]
    # new_dataset["triple_plus_knowledge"] = [[row + [padding] * (max_l_triple_plus_knowcans - len(row)) for row in x] for x in new_dataset["triple_plus_knowledge"]]
    new_dataset["ner_cans"] = [[row + [padding] * (max_l_ner_cans - len(row)) for row in x] for x in new_dataset["ner_cans"]]
...
# TODO : MODIFY to improve Retrieval.
...
def build_input_from_segments_bart_inctxt(persona, knowledge, history, persona_cans, persona_grounding, knowledge_cans,
                                          knowledge_answer_idx, ID, tokenizer, filtered_triple_cans,
                                          ner_cans, lm_labels=False, testset=False, inference=False, with_eos=True):
    ...
    # Only new model training scheme... for now. Fitting knowledge_candidate scheme.
    # TODO : Apply token cleaning to evaluation
    instance["filtered_triples"] = [[bos] + can[:100] + [knowledge_st] + [eos] if len(can) > 100 else [bos] + can + [knowledge_st] + [eos] for can in filtered_triple_cans]
    # instance["filtered_triples_raw"] = filtered_triple_cans_raw
    # instance["triple_plus_knowledge"] = [[bos] + (can + knowledge_cans[i])[:100] + [knowledge_st] + [eos] if len(can) > 100 else [bos] + (can + knowledge_cans[i]) + [knowledge_st] + [eos] for i, can in enumerate(filtered_triple_cans)]
    instance["ner_cans"] = [can[:100] if len(can) > 100 else can for can in ner_cans]
    ...
    for dataset_name, dataset in plan.items():
        ...
        for dialog in dataset:
            ...
            for i, utt in enumerate(utterance):
                ...
                filtered_triple_cans = utt['filtered_triple_candidates']
                ner_cans = utt['ner_knowledge_candidates']
...
def get_testdata_loaders(args, tokenizer, generation=True):
    ...
    plan = get_dataset_only_test(tokenizer, args.test_dataset_path, args.test_dataset_cache, args.without_rel)
    ...
    for dataset_name, dataset in plan.items():
        ...
        for dialog in dataset:
            ...
            for i, utt in enumerate(utterance):
                ...
                filtered_triple_cans = utt['filtered_triple_candidates']
                # filtered_triple_cans_raw = utt['filtered_triple_candidates_raw']
                ner_cans = utt['ner_knowledge_candidates']
                ...
                if model_name == 'GPT2' or model_name == 'transformer-decoder':
                    ...
                    instance = build_input_from_segments_bart_inctxt(persona, knowledge, history, persona_cans, persona_grouding,
                                                         knowledge_cans, knowledge_answer_idx, ID, tokenizer, filtered_triple_cans,
                                                         ner_cans, lm_labels=True, testset=testset)
    ...
    for dataset_name, dataset in datasets.items():
        ...
        for input_name in MODEL_INPUTS:
            # logger.info(f"Loading {input_name}")
            # import pdb;pdb.set_trace()
            tensor = torch.tensor(dataset[input_name], device=args.device)
            tensor_datasets[dataset_name].append(tensor)
