            ...
            with open(file, "r", encoding="utf-8") as f:
                ...
                for line in f:
                    ...
                    for i, utt in enumerate(utterance):
                        ...
                        # Not needed for v5, but needed for v4
                        ...
                        for cans in filtered_triple_can:
                            joined_can = " [SEP] ".join(cans) if isinstance(cans, list) else cans
                            filtered_concat_triple_can.append(joined_can)
                        # print(filtered_concat_triple_can)
...
def get_dataset_only_test(tokenizer, dataset_path, dataset_cache, without_rel):
                ...
                for i, utt in enumerate(utterance):
                    ...
                    for cans in filtered_triple_can:
                        ...
                        if not without_rel:
                            ...
                            triples = [[subjects[i], relations[i], objects[i]] for i in range(len(subjects))]
                        else:
                            sents = [subjects[i] + " " + objects[i] for i in range(len(subjects))]
                            triples = [[subjects[i], objects[i]] for i in range(len(subjects))]
                        filtered_concat_triple_can.append(" ".join(sents))
                        filtered_triple_lists.append(triples)
                    filtered_triple_can_enc = [tokenizer.convert_tokens_to_ids([tok for tok in tokenizer.tokenize(
                        sentence.strip() if sentence else "")]) for sentence in filtered_concat_triple_can]
                    filtered_triple_lists_enc = [[[tokenizer.convert_tokens_to_ids([ \
                        tok for tok in tokenizer.tokenize(phrase.lower().strip() if phrase else "")])
                        for phrase in triple ] for triple in triple_list]
                                                 for triple_list in filtered_triple_lists]
                    # import pdb; pdb.set_trace()
